...
function apply_custom_style()
	...
	colors[clr.TitleBg] = ImVec4(0.177, 0.0, 1, 0.812)
	colors[clr.TitleBgCollapsed] = ImVec4(0, 0.055, 1.0, 0.560)
	colors[clr.TitleBgActive] = ImVec4(0.177, 0.0, 1, 0.812)
	colors[clr.MenuBarBg] = ImVec4(0.177, 0.0, 1, 0.812)
...
function imgui.OnDrawFrame()
  ...
  if second_window.v then
    ...
    if obnova.v then
                local iScreenWidth, iScreenHeight = getScreenResolution()
                imgui.SetNextWindowPos(imgui.ImVec2(iScreenWidth / 2, iScreenHeight / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(7, 3))
                imgui.Begin(u8 'Обновление 2.9.4', obnova, imgui.WindowFlags.NoResize, imgui.WindowFlags.NoCollapse)
				imgui.BeginChild("Обновление 2.9.4", imgui.ImVec2(490, 250), true, imgui.WindowFlags.VerticalScrollbar)
				...
				imgui.BulletText(u8 '3.Удалено проктически все что связнано с Instructors Helper.')
                imgui.BulletText(u8 '4.Переработаны задержки всех отыгровок.')
				imgui.BulletText(u8 '5.Устранены мелкие баги и проблемы с текстом')
				imgui.BulletText(u8 '6.Изменен цвет интерфейса')
				imgui.Separator()
                imgui.BulletText(u8 'Связь и предложения:')
                imgui.BulletText(u8'ВК(vk.com/mark_kaufmann)')
