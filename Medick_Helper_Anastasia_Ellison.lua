...
function main()
  ...
  if seshsps == 1 then
	...
	ftext('Глав.Врач: Makar Sheludkov.',-1)
	ftext('Скрипт редактировал: Doni Baerra.',-1)
	ftext('Функции скрипта команда и кнопка: {ff0000} /Ih и F3.',-1)
...
{
  {
  ...
  },
   {
	...
	sampSendChat("Приветствую Вас в Министерстве Здравоохранения.")
      ...
      {
    title = "{80a4bf}»{FFFFFF} Лекция ADD New {139BEC}",
    onclick = function()
	local _, myid = sampGetPlayerIdByCharHandle(PLAYER_PED)
        local myname = sampGetPlayerNickname(myid)
        sampSendChat("Всех приветствую. Я сотрудник Больницы "..myname:gsub('_', ' ')..". ")
        wait(1300)
        sampSendChat("/mem")
        wait(1300)
        sampSendChat("/mem")
...
function imgui.OnDrawFrame()
  ...
  if second_window.v then
    ...
    imgui.SameLine()
    if imgui.Button(u8 'Сообщить об ошибке/баге', imgui.ImVec2(165, 30)) then os.execute('explorer "https://vk.com/msheludkov2017"')
